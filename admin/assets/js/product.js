...
// Sự kiện tìm kiếm
document.querySelector(".search-filter__button").addEventListener("click", searchProducts);
...
function uploadImage(el) {
    ...
    if (file) {
        ...
        if (file.size > maxSizeInBytes) {
            alert("Dung lượng tệp quá lớn! Vui lòng chọn tệp nhỏ hơn " + (maxSizeInMB * 1000) + "KB.");
        ...
        document.querySelector(".upload-image-preview").setAttribute("src", newPath); // Cập nhật src của hình ảnh xem trước
    }
...
function setDefaultValue() {
    document.querySelector(".upload-image-preview").src = "./assets/img/shopping/blank-image.png";
...
function uploadImageForEdit(input) {
    ...
    if (file) {
        ...
        // Kiểm tra kích thước tệp
        if (file.size > maxSizeInBytes) {
            alert("Dung lượng tệp quá lớn! Vui lòng chọn tệp nhỏ hơn " + maxSizeInMB + "MB.");
...
function editProduct(productId) {
    ...
    if (product) {
        ...
        document.getElementById("edit-gia").value = removeCommas(product.product_price);
        document.getElementById("edit-mo-ta").value = product.desc;
        document.getElementById("edit-image-preview").src = product.product_image || '../admin/assets/img/shopping/blank-image.png'; // Hình ảnh mặc định nếu không có
...
document.getElementById("edit-product-form").addEventListener("submit", function(e) {
    ...
    const products = JSON.parse(localStorage.getItem("products")) || [];
    const productIndex = products.findIndex(p => p.id === window.currentEditProductId);
    ...
    // Cập nhật loại món ăn
    products[productIndex].topic = document.getElementById("edit-chon-mon").value; // Sửa từ category thành topic

    // Cập nhật lại hình ảnh nếu có
    let newImageSrc = getPathImage(document.querySelector("#edit-image-preview").src);
